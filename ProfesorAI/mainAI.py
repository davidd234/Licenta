...
SYSTEM_PROMPT = """
Esti Profesor AI, un asistent educational integrat intr-un proiect GNS3 de retea.
...
def ask_openai(question):
    if not OPENAI_API_KEY or OPENAI_API_KEY == "PASTE_OPENAI_KEY_HERE":
        return (
            "Cheia OpenAI nu este configurata. "
            "Creeaza fisierul .env si adauga OPENAI_API_KEY=cheia_ta."
        )
    ...
    try:
        ...
        return f"Eroare la conectarea catre OpenAI API: {exc}"
...
def start_api_server():
    if is_server_running():
        print(f"\nServerul API ruleaza deja pe portul {SERVER_PORT}.")
        return
    ...
    print(f"Adresa locala: http://127.0.0.1:{SERVER_PORT}")
    print(f"Endpoint intrebari: http://127.0.0.1:{SERVER_PORT}/ask")
...
def stop_api_server():
    if os.path.exists(PID_FILE):
        try:
            ...
            print(f"\nNu am putut opri serverul prin PID file: {exc}")

    # Fallback pentru Linux
    if os.name != 'nt':
        subprocess.run(
            ["pkill", "-f", f"{os.path.basename(__file__)} --server"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )
    print("\nComanda de oprire a fost trimisa.")
...
def show_server_log():
    ...
    try:
        with open(SERVER_LOG, "r", encoding="utf-8") as file:
